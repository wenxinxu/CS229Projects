...
data = {
    'tra': pd.read_csv('data/train.csv', dtype=dtypes, parse_dates=['date']),
    ...
    'hol': pd.read_csv('data/holidays_events.csv', dtype={'transferred': str}, parse_dates=['date']),
...
for method in range(1, number_regressors_to_test + 1):
    ...
    ra1 = round(method + 515 * method + 56 * method)
    np.random.seed(ra1)
    ...
    if (method == 5):
        ...
        r = GradientBoostingRegressor(n_estimators=85, max_depth=6, learning_rate=0.04,
    ...
    ra2 = round(method + 331 * method + 561 * method)
    np.random.seed(ra2)
    ...
    test['unit_sales'] = (np.exp(test['unit_sales']) - 1).clip(
        lower=cut)  # adopted in https://www.kaggle.com/the1owl/forecasting-favorites , version 10
